...
Graph_01 <- cod_landings_total %>% 
 ...
 #   scale_y_continuous(breaks = seq(0,326000, 50000))+
    labs(title = "Atlantic and Pacific Cod Landings From 1950 to 2021",
         x = "Years",
...
# technically to add reference line but not working...
#Graph_01 <- layout(Graph_01, shapes = list(
 # list(type = "line",
  #     x0 = 1992,
   #    y0 = 0,
    #   x1 = 1992,
...
```{r area-or-bar-chart-total-pounds}
#look at why it doesnt work when you put area instead of column
#remove the faceted to have a stacked area chart or bar char
#figure out axis?
#add labels
#add animation
# figure out the error in seq default
#add the other year x-axis
...
fish_coords <- cod_landings_total %>%
 ...
 cod_anim <- ggplot(mapping = aes(
         x = Year,
         y = Total_Pounds/1000000)) +
  geom_col(data = cod_landings_total, aes(fill = Fishing_Types))+
  geom_point(data = fish_coords) +
  geom_image(data = fish_coords, aes(image = cod_image), size = 0.05, alpha = 0.3) + 
    scale_x_continuous(breaks = seq(1950, 2021, 10))+
    #scale_y_continuous(breaks = seq(40000000,730000000, 500000000))+
  facet_wrap (~ Common_Name, nrow=2)+
  scale_fill_viridis_d() +
   labs(title = "Pacific and Antlantic Cod Landings by Fishing Type",
